...
The fastest way to start using better fetch is to import the `betterFetch` function and start making requests and define the response type using generics or output **zod schema (recommended)**.
...
2. **By passing a generic type to the `betterFetch`**: You can also pass a generic type to the `betterFetch` function. This is useful when you want to define the type of the response data in a single place.
