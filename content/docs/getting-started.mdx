...
The easiest way to start using better fetch is to import the `betterFetch` function and start making requests.

```ts twoslash title="Using Generic Type"
...
```ts twoslash title="Using Zod Schema"
import { betterFetch } from '@better-fetch/fetch';
import { z } from 'zod';
